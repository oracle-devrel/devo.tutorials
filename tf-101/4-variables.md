...
This lesson will take a deeper look at Terraform variabales.  You've already had a little bit of exposure to them in the [Experiencing Terraform](2-experiencing-terraform.md) tutorial, as well as a brief summary in the [Understanding Terraform Concepts](3-understanding-terraform.md) tutorial.  Let's dive in and take a deeper look at what these are and how they're used.

## Why use variables?

Variables provide a way to easily decouple a value and its reference from your Terraform code.  A topology may be defined, but the specifics given programmatically (or manually).  Writing parameterized Terraform code (using variables for many of the customizable values), as an environment changes, new values for the variables may be provided, without requiring any modification to the underlying Terraform code.
...
There's another kind of variable called local variables (aka locals) that are not accessible to your entire project, but are limited to the context of a single Terraform file.  This means that if you define a local variable in a particular Terraform file, it will only be available to code within that same file (but not to code in other Terraform files).
...
If this looks familiar, it should - it's borrowed from the [Experiencing Terraform](2-experiencing-terraform.md) tutorial!  This is how the `tenancy_ocid` variable is referenced (read and used).
...
Local values are a little different than regular variables (which you'll be looking at in the rest of this section).  Locals (aka local values) are set in the Terraform code itself.  A benefit to locals is that because they are set within the Terraform code, they can be computed programmatically, granting the ability to use Terraform functions, reference other variables, locals and resources.
...
You can set one or more variables using this technique.  This is one way to set variables, but it is a bit tedious, because it can result in a really long command when running Terraform (if there are lots of variables being set).  This would need to be used every time you want to run Terraform!
...
These are setting variable values (note how there's no `var.` prefix used).  Terraform implicitly uses the `terraform.tfvars` file for setting variable values.
...
You can specify as many files as you'd like that end in `.auto.tfvars` and Terraform will gladly read these and set variable values accordingly.  This allows for the setting different variable definitions between different files.  For instance, it might make sense to the variable definitions as-follows (this is just an example, by no means the only way).
...
This is annoying and can be tedious (especially if there are lots of undefined variables), however there are times when this might make perfect sense and be a desirable behavior.  In situations where the user should be asked for a value (such as for a confirmation prompt), this might be what's needed.
...
In the above example, unless a value is explicitly provided (overriding the default), the default value of "abcd.1234" will be used for the the `compartment_id` variable.

### Sensitive variables
If you set the sensitive attribute of a variable to true, Terraform tries to minimize displaying the value to the user (screen). This is not a guarantee it's not accessible to the user (or that it won't be shown on the screen).  See the [Terraform documentation](https://www.terraform.io/docs/language/values/variables.html#suppressing-values-in-cli-output) for more information.
...
Here's an example of a more complex variable (taken from [https://github.com/oracle-devrel/terraform-oci-ocloud-landing-zone/blob/main/component/network_domain/input.tf](https://github.com/oracle-devrel/terraform-oci-ocloud-landing-zone/blob/main/component/network_domain/input.tf)):
