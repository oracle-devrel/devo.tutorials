...
## Prerequisites
You should have an Oracle Cloud Infrastructure (OCI) account setup.  [Click here](https://www.oracle.com/cloud/free/?source=:ow:o:s:nav::DevoGetStarted&intcmp=:ow:o:s:nav::DevoGetStarted) to create a new cloud account.
...
When spreading the code across many different files, usually resources are grouped in a logical manner.  This could be by an arbitrary category of resources, though one favorite technique is to utilize a file name that shares the resource name and place all of the given type of resourcces in it.  For an example, all compartments could be defined in `compartments.tf`, all Security Lists be defined in `security_lists.tf`, etc.  A key takeaway here is to think about your environment and try to identify a logical way to represent the code itself.  It should be intuitive and fairly easy to navigate (find resources within the project).
...
For simplicity, we'll use just a single file, `main.tf`.  This isn't recommended beyond a super simple environment, but that's what we'll be doing now, so will work just fine.
...
Let's go ahead and initalize Terraform now for this project:
...
When you first created the `main.tf` Terraform file – this was just a plain-text file.  Terraform code is just plain text!  It’s easy to create and manage Terraform code from virtually anywhere.
...
Although there are plenty of ways to programmatically generate Terraform code, Terraform code is often created and managed by people.  People make mistakes.  This can be disastrous.  An easy way to look for egregious Terraform code syntax violations is to use the validation built into Terraform.  To see this in action, run `terraform validate`.
...
You aren’t told that it’s an invalid region, however OCI’s unable to authenticate you properly (because you’re trying to talk to a region that you're not subscribed to).
...
You've had a chance to look at how to create new OCI resources with Terraform.  [Next up](6-changing.md) you’ll learn about [how to modify OCI infrastructure using Terraform](6-changing.md).
